    ...
    if inspection.diagnostics is not None:
        ...
        lines.extend(
            (
                ...
                "- Position-topology projection: "
                f"{sum(item.virtual_weld_boundary_edge_count for item in primitives)} boundary "
                "edges, "
                f"{sum(item.virtual_weld_non_manifold_edge_count for item in primitives)} "
                "non-manifold edges, "
                f"{sum(item.virtual_weld_inconsistent_winding_edge_count for item in primitives)} "
                "inconsistent shared edges",
                "- Vertex representation: "
                f"{sum(item.unused_position_count for item in primitives)} unused positions; "
                f"{sum(item.protected_duplicate_count for item in primitives)} expected "
                "attribute-seam splits; "
                f"{sum(item.attribute_safe_merge_count for item in primitives)} "
                "losslessly compactable duplicate tuples; "
                "FIFO-16 ACMR range "
