...
**Current commit:** 444a99877a01694020209d22dab831660c84a47e
...
| M4 Inspector | COMPLETE | Broken fixture yields all 10 expected defects; clean has no false severe/auto-safe findings; deterministic CLI artifacts | This commit | Completed 2026-08-21 |
| M5 Repair engine | IN_PROGRESS |  |  |  |
...
Generate registered repair candidates, validate authorization, and safely apply renames and the approved root normalization transform.

## Latest evidence

- Tests: `uv run pytest` — 12 passed, including M4 inspector acceptance and failure paths.
...
- Demo command or URL: `uv run asset-shepherd inspect fixtures/broken_robot.glb --profile profiles/unreal_indie_robot.json --output <directory>`.
- Generated artifacts: Deterministic `inspection.json` and `report.md`.
...
Complete M5 deterministic candidate planning, decision validation, repair application, and idempotence tests.
