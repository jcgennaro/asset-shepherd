...
- D108 private alarm notification route: `asset-shepherd-operations` reached `UPDATE_COMPLETE` with
  ...
  one enabled action on that topic and remain `OK`; the email subscription is
  `PendingConfirmation`. The address is supplied only through a `NoEcho` parameter and is absent
  from repository content and stack outputs. Recipient confirmation and a bounded delivery proof
  remain.
...
M9 still requires a Step 7 multi-user access choice, confirmation and delivery proof for the
deployed alarm destination, and the complete Step 8 remote case matrix. The submission also remains
...
Confirm and prove the deployed private alarm subscription, then choose and implement the free
judge-access identity boundary before running the complete Step 8 browser matrix.
