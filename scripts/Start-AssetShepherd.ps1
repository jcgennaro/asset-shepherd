...
if ($modelProvider -eq 'meta') {
    if ([string]::IsNullOrWhiteSpace($env:ASSET_SHEPHERD_MODEL_ID)) {
        throw 'Meta mode requires ASSET_SHEPHERD_MODEL_ID.'
    }
    if ($env:OS -ne 'Windows_NT') {
        throw 'This local key helper requires Windows data protection.'
    }
    Add-Type -AssemblyName System.Security
    if (-not (Test-Path -LiteralPath $MetaSecretPath -PathType Leaf)) {
        throw 'No saved Meta Model API key was found. Run .\scripts\Save-MetaModelKey.ps1 first.'
    }
    ...
    try {
        $protectedBytes = [Convert]::FromBase64String(
            [IO.File]::ReadAllText($MetaSecretPath, [Text.Encoding]::UTF8).Trim()
        )
        $plainBytes = [System.Security.Cryptography.ProtectedData]::Unprotect(
            $protectedBytes,
            $null,
        ...
        $plainKey = [Text.Encoding]::UTF8.GetString($plainBytes)
        if ([string]::IsNullOrWhiteSpace($plainKey)) {
            throw 'The saved Meta Model API key is empty. Save it again.'
        }

        $env:MODEL_API_KEY = $plainKey
        Remove-Item Env:OPENAI_API_KEY -ErrorAction SilentlyContinue
        if (-not $hadPreviousIntakeProvider) {
        ...
        throw 'The saved Meta Model API key cannot be unlocked by this Windows user. Save it again.'
    }
    finally {
        if ($hadPreviousModelKey) {
            $env:MODEL_API_KEY = $previousModelKey
